...
for (s in names(n_by_state)) {
  ...
  p5 <- mk_panel(hex_v5 |> filter(state_code == s), xlim, ylim,
                 "v5 — balanced absorption")
  p6 <- mk_panel(hex_v6 |> filter(state_code == s), xlim, ylim,
  ...
  fname <- sprintf("state_pages/v6_%s_%s.png", s, ab)
