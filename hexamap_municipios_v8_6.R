...
# Output: data/mxmunicipio_hex_sf_v8_6.rds (full) or /tmp/v86_states.rds.
...
if (is.null(ONLY)) {
  ...
  saveRDS(hex_v8, "data/mxmunicipio_hex_sf_v8_6.rds")
  cat(sprintf("Saved data/mxmunicipio_hex_sf_v8_6.rds (%d cells)\n", nrow(hex_v8)))
} else {
  saveRDS(hex_v8, "/tmp/v86_states.rds")
  cat(sprintf("Saved /tmp/v86_states.rds (%d cells)\n", nrow(hex_v8)))
}
