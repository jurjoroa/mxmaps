...
# v8.2: BC/BCS territorial-bias fix — exhaustive search kept, lambda
# rebalanced to 0.5x spacing (v8 level) so cohesion gains do not cost
# displacement. v8.1: exhaustive arrangement search for small states (n<=9) + wider,
# higher-lambda post-CVT search — positional accuracy first.
...
# Output: data/mxmunicipio_hex_sf_v8_3.rds (full) or /tmp/v83_states.rds.
...
gamma_override <- c("02" = 0.35,   # Baja California (Ensenada dominates)
...
# All permutations of 1..n as a matrix (n! rows) — small n only
...
for (s in state_codes) {
  ...
  # Auto position anchor: sparse states (large cs) get strong anchoring;
  # dense states none (they need seed mobility for equalization)
  ...
  if (n_target <= 9) {
    # exhaustive: provably best arrangement under displacement + cohesion
  ...
  # --- Coverage repair: anchored CVT can leave gaps; union every uncovered
  # piece into its nearest cell so the state stays fully tiled --------------
  ...
  if (length(uncovered) > 0 && poly_area(uncovered) > 1e-10) {
    up <- st_sfc(uncovered, crs = 4326)
    upieces <- st_cast(up, "POLYGON")
    ...
    cat(sprintf("  state %s: coverage repair filled %.1f%% of state area\n",
                s, 100 * poly_area(uncovered) / st_area_deg2))
  }
  for (i in seq_len(n_target)) {
  ...
  lambda2 <- 0.5 * spacing
  dmat_cp <- sqrt(outer(mun_xy[, 1], cell_pos[, 1], "-")^2 +
                  outer(mun_xy[, 2], cell_pos[, 2], "-")^2)
  cadj_m <- as.matrix(cadj); diag(cadj_m) <- TRUE
  if (n_target <= 9) {
    pm <- all_perms(seq_len(n_target))
    disp_sum <- rowSums(matrix(dmat_cp[cbind(rep(seq_len(n_target), each = nrow(pm)),
                                      as.vector(t(pm)))],
                           nrow = nrow(pm)))
    vp <- rep(0, nrow(pm))
    for (k in seq_len(n_target)) {
      for (j in st_pairs[[k]]) {
        if (j <= k) next
        vp <- vp + as.numeric(!cadj_m[cbind(pm[, k], pm[, j])])
      }
    }
    assigned2 <- pm[which.min(disp_sum + lambda2 * vp), ]
  } else {
    ls2 <- local_search(seq_len(n_target), cell_pos, st_pairs, mun_xy,
                        lambda2, 1.4 * spacing, max_pass = 40)
...
if (is.null(ONLY)) {
  ...
  saveRDS(hex_v8, "data/mxmunicipio_hex_sf_v8_3.rds")
  cat(sprintf("Saved data/mxmunicipio_hex_sf_v8_3.rds (%d cells)\n", nrow(hex_v8)))
} else {
  saveRDS(hex_v8, "/tmp/v83_states.rds")
  cat(sprintf("Saved /tmp/v83_states.rds (%d cells)\n", nrow(hex_v8)))
}
