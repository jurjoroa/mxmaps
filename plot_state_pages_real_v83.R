...
# Output: state_pages/real_v83_<code>_<abbr>.png + real_v83_national.png

setwd("/Users/jorgeroa/Documents/GitHub/mxmaps")
dir.create("state_pages", showWarnings = FALSE)

v7 <- readRDS("data/mxmunicipio_hex_sf_v8_3.rds")
...
ggsave("state_pages/real_v83_national.png", p,
       width = 16, height = 7, dpi = 150, bg = "white")
cat("Wrote state_pages/real_v83_national.png\n")
...
for (s in sort(unique(mxmun_sf$state_code))) {
  ...
  p <- p7 + pr + plot_layout(guides = "collect") +
    plot_annotation(
      ...
      theme = theme(plot.title = element_text(face = "bold", size = 18),
                    plot.subtitle = element_text(color = "grey40", size = 12)))
  fname <- sprintf("state_pages/real_v83_%s_%s.png", s, ab)
