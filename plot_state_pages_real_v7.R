...
# Output: state_pages/real_v7_<code>_<abbr>.png + real_v7_national.png
...
p <- mk_panel(hex_v7, real, c(-118.5, -86.5), c(14, 32.8),
  ...
  plot_annotation(
    title = "Mexico — v7 hex cartogram vs true territory",
    subtitle = "Share of population speaking an indigenous language, 2020 census",
    theme = theme(plot.title = element_text(face = "bold", size = 18),
                  plot.subtitle = element_text(color = "grey40", size = 12)))
ggsave("state_pages/real_v7_national.png", p,
       width = 16, height = 7, dpi = 150, bg = "white")
cat("Wrote state_pages/real_v7_national.png\n")
...
for (s in sort(unique(mxmun_sf$state_code))) {
  ...
  p7 <- mk_panel(hex_v7 |> filter(state_code == s), NULL, xlim, ylim,
                 "v7 — hex cartogram", show_real = FALSE)
  ...
  p <- p7 + pr + plot_layout(guides = "collect") +
    plot_annotation(
      ...
      subtitle = "v7 hex cartogram vs true municipio territory",
      theme = theme(plot.title = element_text(face = "bold", size = 18),
                    plot.subtitle = element_text(color = "grey40", size = 12)))
  fname <- sprintf("state_pages/real_v7_%s_%s.png", s, ab)
