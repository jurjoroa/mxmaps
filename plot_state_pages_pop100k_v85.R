...
# True territory vs v8.3 hex cartogram, same fill, same legend.
# Output: state_pages/pop100k_v85_<code>_<abbr>.png + pop100k_v85_national.png

setwd("/Users/jorgeroa/Documents/GitHub/mxmaps")
dir.create("state_pages", showWarnings = FALSE)

v8 <- readRDS("data/mxmunicipio_hex_sf_v8_5.rds")
...
for (s in sort(unique(mxmun_sf$state_code))) {
  ...
  p8 <- mk_panel(hex_v8 |> filter(state_code == s), xlim, ylim,
                 "v8.5 — hex cartogram", "pop100k_state", FALSE)
  ...
  p <- p8 + pr + plot_layout(guides = "collect") +
    plot_annotation(
      ...
      theme = theme(plot.title = element_text(face = "bold", size = 18),
                    plot.subtitle = element_text(color = "grey40", size = 12)))
  fname <- sprintf("state_pages/pop100k_v85_%s_%s.png", s, ab)
