...
# True territory vs the v8.8 hex cartogram, same fill, same legend.
# Output: state_pages/pop100k_v88_<code>_<abbr>.png + pop100k_v88_national.png

setwd("/Users/jorgeroa/Documents/GitHub/mxmaps")
dir.create("state_pages", showWarnings = FALSE)

v8 <- readRDS("data/mxmunicipio_hex_sf_v8_8.rds")
...
p8 <- mk_panel(hex_v8, c(-118.5, -86.5), c(14, 32.8),
               "v8.8 — best layout (cohesion 78.1%, displacement 22 km)", "pop100k_nat", FALSE)
pr <- mk_panel(real, c(-118.5, -86.5), c(14, 32.8),
               "True municipio territory", "pop100k_nat", TRUE)
p <- p8 + pr + plot_layout(guides = "collect") +
  plot_annotation(
    title = "Mexico — inhabitants per 100k (national), v8.8 vs true territory",
    subtitle = "Each municipio's population as a share of the national population, per 100,000. Real state and national outlines preserved exactly.",
    theme = theme(plot.title = element_text(face = "bold", size = 18),
                  plot.subtitle = element_text(color = "grey40", size = 12)))
ggsave("state_pages/pop100k_v88_national.png", p,
       width = 18, height = 8, dpi = 150, bg = "white")
cat("Wrote state_pages/pop100k_v88_national.png\n")
...
for (s in sort(unique(mxmun_sf$state_code))) {
  ...
  p8 <- mk_panel(hex_v8 |> filter(state_code == s), xlim, ylim,
                 "v8.8 — best layout (cohesion 78.1%, displacement 22 km)", "pop100k_state", FALSE)
  ...
  p <- p8 + pr + plot_layout(guides = "collect") +
    plot_annotation(
      ...
      theme = theme(plot.title = element_text(face = "bold", size = 18),
                    plot.subtitle = element_text(color = "grey40", size = 12)))
  fname <- sprintf("state_pages/pop100k_v88_%s_%s.png", s, ab)
  ggsave(fname, p, width = 17, height = 9, dpi = 150, bg = "white")
  cat(sprintf("Wrote %s\n", fname))
}
