...
# Output: state_pages/real_v82_<code>_<abbr>.png + real_v82_national.png

setwd("/Users/jorgeroa/Documents/GitHub/mxmaps")
dir.create("state_pages", showWarnings = FALSE)

v7 <- readRDS("data/mxmunicipio_hex_sf_v8_2.rds")
...
for (s in sort(unique(mxmun_sf$state_code))) {
  ...
  p7 <- mk_panel(hex_v7 |> filter(state_code == s), NULL, xlim, ylim,
                 "v8.2 — anchored weighted cartogram", show_real = FALSE)
  ...
  p <- p7 + pr + plot_layout(guides = "collect") +
    plot_annotation(
      ...
      subtitle = "v8.2 anchored weighted hex cartogram vs true municipio territory",
      theme = theme(plot.title = element_text(face = "bold", size = 18),
                    plot.subtitle = element_text(color = "grey40", size = 12)))
  fname <- sprintf("state_pages/real_v82_%s_%s.png", s, ab)
