...
# One comparison page per state: v3.1 | v5, high resolution, with legend.
# Output: state_pages/v5_<code>_<abbr>.png (32 pages)
...
mk_panel <- function(hex_data, borders_df, xlim, ylim, title) {
  ggplot() +
    ...
    scale_fill_distiller(palette = "YlGnBu", direction = 1,
                         ...
                         na.value = "grey90") +
    geom_sf(data = borders_df, fill = NA, color = "black", linewidth = 0.5) +
...
for (s in names(n_by_state)) {
  ...
  p31 <- mk_panel(hex_v31 |> filter(state_code == s), borders, xlim, ylim,
                  sprintf("v3.1 — nearest absorption"))
  p5  <- mk_panel(hex_v5 |> filter(state_code == s), borders, xlim, ylim,
                  sprintf("v5 — space balanced to smallest cells"))
  p <- p31 + p5 + plot_layout(guides = "collect") +
    plot_annotation(
      title = sprintf("%s (%s) — %d municipios",
                      nm, ab, n_by_state[s]),
      subtitle = "Municipio hex cartogram QA — black line = true state border",
      theme = theme(plot.title = element_text(face = "bold", size = 18),
                    plot.subtitle = element_text(color = "grey40", size = 12)))
  fname <- sprintf("state_pages/v5_%s_%s.png", s, ab)
