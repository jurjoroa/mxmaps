...
# Output: data/mxmunicipio_hex_sf_v8_2.rds (full) or /tmp/v82_states.rds.
...
for (s in state_codes) {
  ...
  # Sparse-state anchor: pull seeds toward true centroids (positions first)
  anchor <- if (s %in% c("02", "03")) 0.5 else 0
...
if (is.null(ONLY)) {
  ...
  saveRDS(hex_v8, "data/mxmunicipio_hex_sf_v8_2.rds")
  cat(sprintf("Saved data/mxmunicipio_hex_sf_v8_2.rds (%d cells)\n", nrow(hex_v8)))
} else {
  saveRDS(hex_v8, "/tmp/v82_states.rds")
  cat(sprintf("Saved /tmp/v82_states.rds (%d cells)\n", nrow(hex_v8)))
}
