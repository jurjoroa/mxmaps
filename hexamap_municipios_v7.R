...
# v7: two refinements over v6's equal-area CVT cells.
#
...
# Output: data/mxmunicipio_hex_sf_v7.rds (full run) or /tmp for subsets.
...
if (is.null(ONLY)) {
  ...
  saveRDS(hex_v7, "data/mxmunicipio_hex_sf_v7.rds")
  cat(sprintf("Saved data/mxmunicipio_hex_sf_v7.rds (%d cells)\n", nrow(hex_v7)))
} else {
  saveRDS(hex_v7, "/tmp/v7_states.rds")
  cat(sprintf("Saved /tmp/v7_states.rds (%d cells)\n", nrow(hex_v7)))
}
