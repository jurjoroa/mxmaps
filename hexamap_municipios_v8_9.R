...
# Output: data/mxmunicipio_hex_sf_v8_9.rds (full) or /tmp/v89_states.rds.
...
for (s in state_codes) {
  ...
  for (anchor in cands) {
    ...
    if (best_coh > tune_best$coh) {
      tune_best <- list(coh = best_coh, anchor = anchor, best = best)
    }
  }
  best <- tune_best$best
  cat(sprintf("  -> state %s tuned anchor = %.2f (of %s), cohesion %.1f%%\n",
              s, tune_best$anchor,
              paste(sprintf("%.2f", cands), collapse = "/"),
              100 * tune_best$coh))   # best_coh is a FRACTION in this codebase
...
if (is.null(ONLY)) {
  ...
  saveRDS(hex_v8, "data/mxmunicipio_hex_sf_v8_9.rds")
  cat(sprintf("Saved data/mxmunicipio_hex_sf_v8_9.rds (%d cells)\n", nrow(hex_v8)))
} else {
  saveRDS(hex_v8, "/tmp/v89_states.rds")
  cat(sprintf("Saved /tmp/v89_states.rds (%d cells)\n", nrow(hex_v8)))
}
