...
# v8.10: per-state anchor tuning selected on the COMPOSITE (fewest starved
# cells, then cohesion) rather than on cohesion alone.
# v8.9: the anchor is TUNED PER STATE, selected on measured cohesion.
...
# Output: data/mxmunicipio_hex_sf_v8_10.rds (full) or /tmp/v810_states.rds.

setwd("/Users/jorgeroa/Documents/GitHub/mxmaps")
ANCHOR_GRID <- { v <- Sys.getenv("V89_GRID")
  if (nzchar(v)) as.numeric(strsplit(v, ",")[[1]]) else c(0, 0.30, 0.50, 0.70) }
...
for (s in state_codes) {
  ...
  # ---- v8.9: PER-STATE ANCHOR TUNING -------------------------------------
  # The anchor floor is the knob that actually moves the numbers: v8.8 showed
  # a single global floor of 0.30 lifting national cohesion 73.2% -> 78.1% and
  # halving mean displacement 41 -> 22 km. A single value cannot be right for
  # 32 states with a 223x cell-size range, so search it per state and select on
  # the MEASURED cohesion -- the same quantity evaluate_states.R reports.
  #
  # This is per-state tuning done the defensible way. It is a deterministic
  # optimisation over 2469 FIXED municipios, not a learning problem, so there is
  # no overfitting risk. What made v8.5's per-state constants indefensible was
  # that they were picked by eye and the reported figure was a per-state number
  # quoted as national -- not that they varied per state.
  #
  # Candidates that do not change `anchor` for this state are skipped: for a
  # sparse state the natural value already exceeds the floor, so the floor is
  # inert and only one candidate is evaluated.
  nat_anchor <- pmax(pmin(0.35 * (cs_ratio - 1), 0.7), 0)
  cands <- sort(unique(pmax(nat_anchor, ANCHOR_GRID)))
  tune_best <- list(coh = -1)
  # PATH DEPENDENCE GUARD. The candidate loop reads P and w and then overwrites
  # them (`P <- r3$P; w <- r3$w`), so without this every candidate after the
  # first started from the PREVIOUS candidate's converged state. That makes the
  # loop a chain rather than a sweep, and makes the selected artifact
  # irreproducible from the reported anchor -- precisely the v8.5 failure, by a
  # different mechanism. Snapshot the entry state and restore it per candidate.
  P0 <- P; w0 <- w
  for (anchor in cands) {
    P <- P0; w <- w0
    ...
    # SELECT ON THE COMPOSITE, NOT ON COHESION. Choosing the highest-cohesion
    # anchor per state produced 78.9% national cohesion with 60 cells under a
    # quarter of their state mean and 19 under a tenth -- the worst legibility
    # tail of any version, for +0.4 cohesion. Legibility of every municipio is
    # the entire purpose of the layout, so a candidate that creates invisible
    # cells is a regression however well it scores on adjacency. Rank
    # lexicographically: fewest starved cells first, then cohesion.
    cand_a <- vapply(best$cells, function(g)
      if (is.null(g)) 0 else poly_area(st_sfc(g, crs = 4326)), 0)
    cand_a <- cand_a[cand_a > 0]
    n_tiny_c <- sum(cand_a / mean(cand_a) < 0.25)
    better <- is.null(tune_best$n_tiny) ||
      n_tiny_c < tune_best$n_tiny ||
      (n_tiny_c == tune_best$n_tiny && best_coh > tune_best$coh)
    if (better) {
      tune_best <- list(coh = best_coh, anchor = anchor, best = best,
                        n_tiny = n_tiny_c)
    }
  }
  best <- tune_best$best
  cat(sprintf("  -> state %s tuned anchor = %.2f (of %s), cohesion %.1f%%, n_tiny %d\n",
              s, tune_best$anchor,
              paste(sprintf("%.2f", cands), collapse = "/"),
              100 * tune_best$coh, tune_best$n_tiny))  # coh is a FRACTION here
...
if (is.null(ONLY)) {
  ...
  saveRDS(hex_v8, "data/mxmunicipio_hex_sf_v8_10.rds")
  cat(sprintf("Saved data/mxmunicipio_hex_sf_v8_10.rds (%d cells)\n", nrow(hex_v8)))
} else {
  saveRDS(hex_v8, "/tmp/v810_states.rds")
  cat(sprintf("Saved /tmp/v810_states.rds (%d cells)\n", nrow(hex_v8)))
}
