# ---------------------------------------------------------------------------
# Purpose: v13 — v12 plus PER-STATE MULTI-START, selected on measured cohesion.
#
#          Jorge asked for per-state maximisation rather than one global
#          setting. The important distinction from v8.5's per-state tuning,
#          which shipped a regression as a win: this is a deterministic
#          optimisation over 2469 FIXED municipios, not a learning problem, so
#          there is no overfitting risk and tuning each state as hard as
#          possible is simply correct. What went wrong in v8.5 was that the
#          constants were chosen BY EYE and the reported number was a per-state
#          figure quoted as national. So here every candidate is scored by the
#          same measured cohesion the evaluator reports, the winner is selected
#          automatically, and the winning restart is printed for reproducibility.
#
#          Crucially the search does NOT touch the design parameters. gamma and
#          the area-target clamping stay exactly as designed, and the areas are
#          solved to their targets in every candidate -- so no candidate can buy
#          cohesion by quietly abandoning legibility. The only thing varied is
#          the initial site configuration (a small seeded jitter), which changes
#          which adjacency configuration the solver lands in and nothing else.
#
#          v12 — v11's optimal-transport solver plus CENTROID TARGETING, so
...
#          V13_STARTS=<k>  restarts per state (default 5)
#          V13_JITTER=<f>  jitter as a fraction of local cell size (default 0.18)
# Output:  data/mxmunicipio_hex_sf_v13<TAG>.rds or /tmp/v13_states.rds
# ---------------------------------------------------------------------------
...
STARTS  <- as.integer(.env("V13_STARTS", 5))
JITTER  <- .env("V13_JITTER", 0.18)
...
for (s in state_codes) {
  ...
  P_true <- P
  best_run <- list(coh = -1)
  for (rs in seq_len(STARTS)) {
    # restart 1 is the unperturbed centroid configuration, so v13 can never be
    # worse than v12; later restarts jitter the sites by a fraction of the
    # LOCAL cell size, seeded deterministically from the state code.
    set.seed(1000 * rs + as.integer(s))
    P <- P_true
    if (rs > 1) {
      rad <- JITTER * sqrt(tgt / pi)
      ang <- runif(n, 0, 2 * pi)
      P <- P_true + cbind(rad * cos(ang), rad * sin(ang))
    }
    ...
    # A restart is only ELIGIBLE if its areas actually converged. Without this
    # the search happily picks a candidate that scores well on cohesion while
    # sitting at 68% area error -- i.e. it buys cohesion by abandoning the
    # legibility the areas encode, which is the one thing this design must not
    # do. (Observed on state 01 restart 4.)
    area_ok <- max(abs(rr - 1)) < 0.05
    if (area_ok && 100 * pres / max(tot, 1) > best_run$coh) {
      best_run <- list(coh = 100 * pres / max(tot, 1), cells = b$cells,
                       P = P, rr = rr, outer = outer, rs = rs, keep = keep)
    }
  }
  if (best_run$coh < 0) stop(sprintf("state %s: no restart converged on area", s))
  b <- list(cells = best_run$cells); P <- best_run$P
  rr <- best_run$rr; outer <- best_run$outer; keep <- best_run$keep
  cat(sprintf("  -> state %s best of %d restarts: restart %d, cohesion %.1f%%\n",
              s, STARTS, best_run$rs, best_run$coh))
...
if (is.null(ONLY)) {
  saveRDS(out, sprintf("data/mxmunicipio_hex_sf_v13%s.rds", TAG))
  cat(sprintf("Saved data/mxmunicipio_hex_sf_v13%s.rds (%d cells)\n", TAG, nrow(out)))
} else {
  saveRDS(out, "/tmp/v13_states.rds")
  cat(sprintf("Saved /tmp/v13_states.rds (%d cells)\n", nrow(out)))
}
