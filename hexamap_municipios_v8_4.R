...
# Output: data/mxmunicipio_hex_sf_v8_4.rds (full) or /tmp/v84_states.rds.
...
for (s in state_codes) {
  ...
  for (rd in 1:3) {
    ru <- cvt_run(P, w, tgt, 0.25, 12, 0.5, 1.0, state_clip, sq, mun_xy, anchor, st_pairs)
    ...
    ls <- local_search(assigned2, cell_pos, st_pairs, mun_xy,
                       1.0 * spacing, 1.4 * spacing, max_pass = 15)
...
if (is.null(ONLY)) {
  ...
  saveRDS(hex_v8, "data/mxmunicipio_hex_sf_v8_4.rds")
  cat(sprintf("Saved data/mxmunicipio_hex_sf_v8_4.rds (%d cells)\n", nrow(hex_v8)))
} else {
  saveRDS(hex_v8, "/tmp/v84_states.rds")
  cat(sprintf("Saved /tmp/v84_states.rds (%d cells)\n", nrow(hex_v8)))
}
