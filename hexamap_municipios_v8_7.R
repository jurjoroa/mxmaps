...
# v8.7: MEASURED NEGATIVE RESULT -- keep for the record, do not build on it.
#
# Hypothesis: v8.6 sits ~16 points below the degree-sequence permutation bound
#   max_pi sum_i min(deg_M(i), deg_C(pi(i))) / sum_i deg_M(i)
# (89-98% per state), so a better search should recover much of it. Added
# simulated annealing to escape the 2-opt local optimum.
#
# It does not work, and three experiments say why:
#   * annealing:            Veracruz 64 -> 64.7, Zacatecas 68 -> 68.5
#   * adjacency price:      lambda 1x, 3x, 8x * spacing all give EXACTLY 68.5%
#                           on Zacatecas -- the search is converged, not
#                           mispriced
#   * identity baseline:    Zacatecas identity 64.5% vs searched 68.5% -- the
#                           search adds ~4 points and is then done
#
# So RE-ASSIGNMENT IS EXHAUSTED: ~68-73% is what these cells can support. The
# degree-sequence bound is valid but far too LOOSE to be actionable -- two
# graphs with identical degree sequences can share almost no edges, so 89-98%
# was never reachable by permuting municipios among fixed cells.
#
# The remaining headroom is in the CELL GEOMETRY, not the assignment. Delaunay
# of the true municipio centroids reproduces 83.1% of true adjacency vs v8's
# 73.2%; the CVT moves sites off those centroids to equalise areas and breaks
# edges that were free. By Aurenhammer-Hoffmann-Aronov the prescribed areas are
# achievable by the WEIGHTS ALONE for any fixed sites, so the sites need not
# move at all -- that is the untested lever, and it needs a damped-Newton
# optimal-transport solver (Kitagawa-Merigot-Thibert), not a gradient step.
...
# Output: data/mxmunicipio_hex_sf_v8_7.rds (full) or /tmp/v87_states.rds.

setwd("/Users/jorgeroa/Documents/GitHub/mxmaps")

# LAM_MULT prices one broken adjacency in units of `spacing` (a cell width).
# At 1.0 -- the v8.4/8.5/8.6 value -- a swap that repairs one adjacency costs
# more in displacement than it gains, so the search is near-optimal FOR THAT J
# and the 16-point gap to the permutation bound is a pricing choice, not a
# search failure. Raising it buys cohesion with displacement.
LAM_MULT <- { v <- Sys.getenv("V87_LAM"); if (nzchar(v)) as.numeric(v) else 1.0 }
...
# v8.7: SIMULATED ANNEALING for the assignment.
#
# Why: with v8.6's cells held fixed, the rigorous permutation bound
#   max_pi sum_i min(deg_M(i), deg_C(pi(i))) / sum_i deg_M(i)
# (sorted degree sequences, by the rearrangement inequality) is 89-98% for
# almost every state, while v8.6 achieves 64-90% -- a median 16-point gap.
# So the CELLS are not the constraint; the SEARCH is. local_search accepts only
# strictly improving single transpositions, so it halts at a 2-opt local
# optimum. Annealing accepts uphill moves with probability exp(-dJ/T) and so
# escapes them; the "best-of-3 restarts" v8.5 added for Edomex is the weakest
# possible substitute for this.
#
# Same J as v8.6 -- displacement + lambda * TRUE broken cell adjacency.
anneal_search <- function(assigned, pos, st_pairs, mun_xy, lambda, cadj_m,
                          n_sweep = 140, t_hi = 1.2, t_lo = 0.02,
                          p_far = 0.10) {
  n <- length(assigned)
  inv <- integer(n); inv[assigned] <- seq_len(n)

  Jk <- function(k, a) {
    js <- st_pairs[[k]]
    d <- sqrt(sum((mun_xy[k, ] - pos[a[k], ])^2))
    if (length(js) == 0) return(d)
    d + lambda * sum(!cadj_m[a[k], a[js]])
  }
  Jtot <- function(a) sum(vapply(seq_len(n), Jk, 0, a = a))

  cur <- assigned; curJ <- Jtot(cur)
  best <- cur; bestJ <- curJ
  T0 <- t_hi * lambda; T1 <- t_lo * lambda

  for (sw in seq_len(n_sweep)) {
    Temp <- T0 * (T1 / T0) ^ ((sw - 1) / max(n_sweep - 1, 1))
    for (rep in seq_len(n)) {
      k <- sample.int(n, 1L)
      # neighbourhood: municipios holding a cell adjacent to k's cell -- these
      # are exactly the moves that can change an adjacency. Occasional far
      # moves keep the chain mixing.
      if (runif(1) < p_far) {
        c <- sample.int(n, 1L)
      } else {
        nbc <- which(cadj_m[cur[k], ])
        if (length(nbc) == 0) next
        c <- inv[nbc[sample.int(length(nbc), 1L)]]
      }
      if (is.na(c) || c < 1L || c == k) next
      aff <- unique(c(k, c, st_pairs[[k]], st_pairs[[c]]))
      a2 <- cur; a2[c(k, c)] <- cur[c(c, k)]
      dJ <- sum(vapply(aff, Jk, 0, a = a2)) - sum(vapply(aff, Jk, 0, a = cur))
      if (is.na(dJ)) next
      if (dJ < 0 || runif(1) < exp(-dJ / max(Temp, 1e-12))) {
        cur <- a2
        inv[cur[c(k, c)]] <- c(k, c)
        curJ <- curJ + dJ
        if (curJ < bestJ) { bestJ <- curJ; best <- cur }
      }
    }
  }
  list(assigned = best)
...
for (s in state_codes) {
  ...
  anchor <- pmax(pmin(0.35 * (cs_ratio - 1), 0.7), 0)
  ...
  for (rs in seq_len(n_restart)) {
    ...
    na_rows <- which(!complete.cases(cell_pos))
    if (length(na_rows) > 0) cell_pos[na_rows, ] <- P[na_rows, ]
    an <- anneal_search(assigned2, cell_pos, st_pairs, mun_xy,
                        LAM_MULT * spacing, cadj_m,
                        n_sweep = max(40L, min(200L, as.integer(12000 / n_target))))
    ls <- local_search(an$assigned, cell_pos, st_pairs, mun_xy,
                       LAM_MULT * spacing, cadj_m, 1.4 * spacing, max_pass = 15)
...
if (is.null(ONLY)) {
  ...
  saveRDS(hex_v8, "data/mxmunicipio_hex_sf_v8_7.rds")
  cat(sprintf("Saved data/mxmunicipio_hex_sf_v8_7.rds (%d cells)\n", nrow(hex_v8)))
} else {
  saveRDS(hex_v8, "/tmp/v87_states.rds")
  cat(sprintf("Saved /tmp/v87_states.rds (%d cells)\n", nrow(hex_v8)))
}
