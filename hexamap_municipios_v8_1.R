...
# v8.1: exhaustive arrangement search for small states (n<=9) + wider,
...
# Output: data/mxmunicipio_hex_sf_v8_1.rds (full) or /tmp/v81_states.rds.
...
if (is.null(ONLY)) {
  ...
  saveRDS(hex_v8, "data/mxmunicipio_hex_sf_v8_1.rds")
  cat(sprintf("Saved data/mxmunicipio_hex_sf_v8_1.rds (%d cells)\n", nrow(hex_v8)))
} else {
  saveRDS(hex_v8, "/tmp/v81_states.rds")
  cat(sprintf("Saved /tmp/v81_states.rds (%d cells)\n", nrow(hex_v8)))
}
