...
# Before/after pages for the repaired states: broken v6 | fixed v6.
# States: 14 Jalisco, 15 Edomex, 18 Nayarit, 21 Puebla.
# Output: state_pages/v6fix_<code>_<abbr>.png

setwd("/Users/jorgeroa/Documents/GitHub/mxmaps")

broken <- readRDS("/tmp/v6_broken_full.rds")
fixedv <- readRDS("data/mxmunicipio_hex_sf_v6.rds")
...
broken <- sanitize(broken)
fixedv <- sanitize(fixedv)

hex_b <- broken |> left_join(census, by = "region")
hex_f <- fixedv |> left_join(census, by = "region")
...
for (s in c("14", "15", "18", "21")) {
  ...
  pb <- mk_panel(hex_b |> filter(state_code == s), xlim, ylim,
                 "v6 before — starved/fallback cells")
  pf <- mk_panel(hex_f |> filter(state_code == s), xlim, ylim,
                 "v6 after — rescued + polished")
  p <- pb + pf + plot_layout(guides = "collect") +
    plot_annotation(
      title = sprintf("%s (%s) — repair", nm, ab),
      subtitle = "Municipio hex cartogram QA — black line = true state border",
      theme = theme(plot.title = element_text(face = "bold", size = 18),
                    plot.subtitle = element_text(color = "grey40", size = 12)))
  fname <- sprintf("state_pages/v6fix_%s_%s.png", s, ab)
